...
function calculateSalary() {
    ...
    if (startDayPay && earnedForDays) {
        document.querySelector("#time .yourPay > p > span").innerHTML = `${getCookie('pay')} грн. | ${currents[1] ? (getCookie('pay') / currents[1].rate).toFixed(2) : 'loading...'} € | ${currents[0] ? (getCookie('pay') / currents[0].rate).toFixed(2) : 'loading...'} $ - месяц <br> ${earnedForOneDay.toFixed(2)} грн. | ${currents[1] ? (earnedForOneDay / currents[1].rate).toFixed(2) : 'loading...'} € | ${currents[0] ? (earnedForOneDay / currents[0].rate).toFixed(2) : 'loading...'} $ - день`;
